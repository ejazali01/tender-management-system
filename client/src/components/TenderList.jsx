...
import { Card, Typography, Button, Dialog, DialogBody, DialogFooter } from "@material-tailwind/react";
...
const Table = () => {
  ...
  const excludedKeys = ["__v", "_id", "createdAt", "updatedAt", "bufferTime"];
      ...
      {loading ? (
        ...
        tender && (
          <Card className="relative h-screen w-full overflow-scroll ">
            <table className="w-full min-w-max table-auto text-left">
              ...
              <tbody>
                {tender.map((tenderItem, index) => {
                  ...
                  if (isTimeUp) {
                    remainingTimeClassName = "bg-red-500 text-white font-bold hover:bg-red-400";
                  } else if (isLessThanFiveMinutes) {
                    remainingTimeClassName = "bg-yellow-500 font-bold hover:bg-yellow-400"; // Yellow for less than 5 minutes
                  } else {
                    remainingTimeClassName = "bg-green-500 font-bold hover:bg-green-400";
                  }
                    ...
                    <tr key={tenderItem._id}>
                      ...
                      <td className="border-b">
                        <button
                          ...
                          className={`px-6 py-4 whitespace-nowrap font-normal text-left ${isTimeUp ? "bg-gray-300 cursor-not-allowed" : "bg-green-500 text-white hover:bg-green-400"}`}
                        >
                          Add Bid
                        </button>
                      </td>
            ...
            <div className="sticky absolute bottom-0 z-20 left-0 max-w-40 bg-gray-300 p-1 px-3">
              <span className="text-gray-800">Total Tender: {tender.length}</span>
            </div>
          </Card>
        )
      )}
      <Quatation  open={bidFormOpen} handleClose={handleBidFormClose} tenderId={selectedTenderId} setSelectedTenderId={setSelectedTenderId} />
    </div>
  );
};
