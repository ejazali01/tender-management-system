  ...
  try {
    ...
    if (tender.endTime - now <= 5 * 60 * 1000) {
      ...
      tender.endTime = new Date(
        tender.endTime.getTime() + tender.bufferTime * 60000
      );
      await tender.save();
    }
...
export const allBids = async (req, res) => {
  try {
    const bids = await Bid.find({ tenderId: req.params.tenderId }).sort({
      bidCost: 1,
    });
    res.json(bids);
  } catch (err) {
...
// Endpoint to delete a single or multiple bids
export const deleteSingleOrMultiple = async (req, res) => {
  try {
    await Bid.deleteMany({ _id: { $in: req?.body?.ids } });
    res.status(200).send({ message: 'bids deleted successfully' });
  } catch (error) {
    res.status(500).send({ message: 'Failed to delete bid', error });
  }
}